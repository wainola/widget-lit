...
const createEvmAssetTransfer = async (evmWallet: EvmWallet): Promise<EVMAssetTransfer> => {
  const assetTransfer = new EVMAssetTransfer();
  await assetTransfer.init(evmWallet!.currentProvider!, Environment.TESTNET);
  return assetTransfer;
}

const createSubstrateAssetTransfer = async (substrateWallet: SubstrateWallet): Promise<SubstrateAssetTransfer> => {
